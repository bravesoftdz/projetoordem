...
if DBLookupComboBox3.Text = '' then
  begin
    ShowMessage('O campo[Produto] não pode ficar em branco.');
    DBLookupComboBox3.SetFocus;
    Abort;
  end;
if DBEdit5.Text = '' then
  begin
    ShowMessage('O campo[Quantidade] não pode ficar em branco.');
    DBEdit5.SetFocus;
    Abort;
  end;
DBEdit5.SetFocus;
...
begin
  ...
  Application.MessageBox('O registro foi cancelado com sucesso.','Informação',MB_OK+MB_DEFBUTTON1+MB_ICONERROR);
...
begin
  confExclusao:= Application.MessageBox('Tem certeza que deseja excluir?','Atenção', MB_YESNO+MB_DEFBUTTON2+MB_ICONQUESTION);
  if confExclusao = IDYES then
  begin
    ShowMessage('Registro nº '+DM.ADODSOrdemServiconum_os.AsString+ ' com sucesso.');
...
begin
DM.ADODSOrdemServico.Post;
Application.MessageBox('O registro foi salvo com sucesso.','Informação',MB_OK+MB_DEFBUTTON1+MB_ICONINFORMATION);
